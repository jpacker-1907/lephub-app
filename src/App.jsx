...
const MEETING_TEMPLATES = {
  ...
  shareholder: {
    name: 'Shareholder Meeting', icon: '📊', frequency: 'Quarterly', duration: '90 min', color: '#1a3a5c',
    category: 'family',
    agenda: [
      { id: 'opening', name: 'Opening & Fiduciary Check-In', duration: '5 min', desc: 'Confirm quorum. Record attendance. Remind the group of fiduciary duties. Approve prior meeting minutes.' },
      { id: 'financial-review', name: 'Financial Performance Review', duration: '20 min', desc: 'Key financials: revenue, EBITDA, cash position, debt ratios. Compare to plan and prior year. Valuation update.' },
      { id: 'distribution', name: 'Distribution & Compensation Review', duration: '10 min', desc: 'Dividend/distribution decisions. Executive compensation review. Family employment policy compliance.' },
      { id: 'succession-update', name: 'Succession & Continuity Update', duration: '10 min', desc: 'Where we stand on leadership pipeline, key-person risk, transition timeline, and estate planning progress.' },
      { id: 'governance', name: 'Governance Health Check', duration: '10 min', desc: 'Are governance structures working? Any policies, bylaws, or buy-sell agreements that need updating?' },
      { id: 'resolve', name: 'Resolve (Identify, Clarify, Act)', duration: '25 min', desc: 'Ownership-level issues on the table. Distribution policy changes, governance amendments, liquidity events, strategic direction.' },
      { id: 'action-items', name: 'Action Items & Adjournment', duration: '10 min', desc: 'Recap action items with owners and deadlines. Any formal votes or resolutions. Set next meeting date. Formal close.' },
    ],
  },
  'family-council': {
...
function LEPFrameworkView({ setCurrentView, setActivePillar, moduleProgress = {}, scores }) {
  ...
  const journeySteps = [
    { num: 1, name: 'Discovery', desc: 'One assessment across all five pillars', color: '#34597A', duration: '2–4 weeks' },
    { num: 2, name: 'Pillar Work', desc: 'Structured sessions producing real deliverables', color: '#E05B6F', duration: '6–18 months' },
  ...
  if (activeModule && activeTrack) {
    ...
    return (
      <div style={{ maxWidth: '720px', margin: '0 auto' }}>
        ...
        {/* Video Playlist — conditional (v42): videos when verified, placeholder when empty */}
        <div style={{ ...cardStyle, borderTop: `3px solid ${track.color}20` }}>
          ...
          {(MODULE_VIDEOS[mod.id]?.playlist || []).length === 0 ? (
            ...
            MODULE_VIDEOS[mod.id].playlist.map((video, vi) => (
              <a key={vi} href={`https://www.youtube.com/watch?v=${video.id}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', gap: '14px', padding: '14px', marginBottom: '8px', borderRadius: '12px', border: '1px solid #DDE3EB', background: 'white', textDecoration: 'none', cursor: 'pointer', alignItems: 'center' }}>
